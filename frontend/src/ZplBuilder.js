 ...
 */
import { CELL_DEFAULTS, DEFAULT_DPMM, } from './types';
import { buildOffsets, findContainingMerge, resolveCellRect, } from './components/table/tableGeometry';
export class ZplBuilder {
    ...
    renderTable(table) {
        ...
        if (table.borderDots > 0) {
            out.push(`^FO${ox},${oy}^GB${totalWDots},${totalHDots},${table.borderDots},B,0^FS`);
            ...
            for (let r = 1; r < table.rowHeightsMm.length; r++) {
                ...
                while (segStartCol < table.colWidthsMm.length) {
                    ...
                    while (segEndCol < table.colWidthsMm.length) {
                        ...
                        const swallowed = !!swallowedAbove &&
                    ...
                    if (segEndCol > segStartCol) {
                        ...
                        const widthDot = this.mmToDot(colXs[segEndCol]) - this.mmToDot(colXs[segStartCol]);
                        out.push(`^FO${xStartDot},${yDot}^GB${widthDot},${table.borderDots},${table.borderDots},B,0^FS`);
                    }
            ...
            for (let c = 1; c < table.colWidthsMm.length; c++) {
                ...
                while (segStartRow < table.rowHeightsMm.length) {
                    ...
                    if (segEndRow > segStartRow) {
                        ...
                        const heightDot = this.mmToDot(rowYs[segEndRow]) - this.mmToDot(rowYs[segStartRow]);
                        out.push(`^FO${xDot},${yStartDot}^GB${table.borderDots},${heightDot},${table.borderDots},B,0^FS`);
                    }
        ...
        for (const cell of table.cells) {
            const rect = resolveCellRect(cell.row, cell.col, table.rowHeightsMm, table.colWidthsMm, table.merges);
            if (rect.row !== cell.row || rect.col !== cell.col)
                continue;
            ...
            if (innerWDot <= 0 || innerHDot <= 0)
                continue;
            if (cell.imageSourceDataUrl) {
                if (!cell.imageEncoded)
                    continue;
                const { hexData, bytesPerRow, totalBytes } = cell.imageEncoded;
                out.push(`^FO${innerXDot},${innerYDot}^GFA,${totalBytes},${totalBytes},${bytesPerRow},${hexData}^FS`);
            }
